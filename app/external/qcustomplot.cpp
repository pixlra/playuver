...
QCPLayer::~QCPLayer()
{
  ...
  if( mParentPlot->currentLayer() == this )
    qDebug() << Q_FUNC_INFO << "The parent plot's mCurrentLayer will be a dangling pointer. Should have been set to a "
...
QCPLayoutElement::QCPLayoutElement( QCustomPlot* parentPlot )
    : QCPLayerable( parentPlot ),  // parenthood is changed as soon as layout element gets inserted into a layout
...
void QCPLayoutElement::update( UpdatePhase phase )
{
  if( phase == upMargins )
  {
    if( mAutoMargins != QCP::msNone )
    {
      ...
      foreach( QCP::MarginSide side, QList<QCP::MarginSide>() << QCP::msLeft << QCP::msRight << QCP::msTop
                                                              << QCP::msBottom )
      {
  ...
  switch( mStyle )
  {
    ...
    case esSpikeArrow:
    {
      QPointF points[4] = {pos.toPointF(), ( pos - lengthVec + widthVec ).toPointF(),
                           ( pos - lengthVec * 0.8f ).toPointF(), ( pos - lengthVec - widthVec ).toPointF()};
      painter->setPen( miterPen );
      painter->setBrush( brush );
...
double QCPAxis::pixelToCoord( double value ) const
{
  if( orientation() == Qt::Horizontal )
  {
    ...
    {
      if( !mRangeReversed )
        return pow( mRange.upper / mRange.lower, ( value - mAxisRect->left() ) / (double)mAxisRect->width() ) *
               mRange.lower;
      else
        return pow( mRange.upper / mRange.lower, ( mAxisRect->left() - value ) / (double)mAxisRect->width() ) *
               mRange.upper;
    }
  }
  else  // orientation() == Qt::Vertical
  {
    ...
    {
      if( !mRangeReversed )
        return pow( mRange.upper / mRange.lower, ( mAxisRect->bottom() - value ) / (double)mAxisRect->height() ) *
               mRange.lower;
      else
        return pow( mRange.upper / mRange.lower, ( value - mAxisRect->bottom() ) / (double)mAxisRect->height() ) *
               mRange.upper;
    }
  }
}
...
void QCPAxisPainterPrivate::draw( QCPPainter* painter )
{
  ...
  if( !label.isEmpty() )
  {
    ...
    if( type == QCPAxis::atLeft )
    {
      ...
      painter->rotate( -90 );
      painter->drawText( 0, 0, alignmentRect.height(), labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter,
                         label );
      painter->setTransform( oldTransform );
    }
    else if( type == QCPAxis::atRight )
    {
      ...
      painter->drawText( origin.x(), origin.y() - margin - labelBounds.height(), alignmentRect.width(),
                         labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter, label );
    else if( type == QCPAxis::atBottom )
      painter->drawText( origin.x(), origin.y() + margin, alignmentRect.width(), labelBounds.height(),
  ...
  if( type == QCPAxis::atLeft )
  {
    mAxisSelectionBox.setCoords( origin.x() - selAxisOutSize, alignmentRect.top(), origin.x() + selAxisInSize,
                                 alignmentRect.bottom() );
    mTickLabelsSelectionBox.setCoords( origin.x() - selTickLabelOffset - selTickLabelSize, alignmentRect.top(),
  ...
  {
    mAxisSelectionBox.setCoords( origin.x() - selAxisInSize, alignmentRect.top(), origin.x() + selAxisOutSize,
                                 alignmentRect.bottom() );
    mTickLabelsSelectionBox.setCoords( origin.x() + selTickLabelOffset + selTickLabelSize, alignmentRect.top(),
                                       origin.x() + selTickLabelOffset, alignmentRect.bottom() );
    mLabelSelectionBox.setCoords( origin.x() + selLabelOffset + selLabelSize, alignmentRect.top(),
  ...
  {
    mAxisSelectionBox.setCoords( alignmentRect.left(), origin.y() - selAxisOutSize, alignmentRect.right(),
                                 origin.y() + selAxisInSize );
    mTickLabelsSelectionBox.setCoords( alignmentRect.left(), origin.y() - selTickLabelOffset - selTickLabelSize,
  ...
  {
    mAxisSelectionBox.setCoords( alignmentRect.left(), origin.y() - selAxisInSize, alignmentRect.right(),
                                                                              ...
                                                                              const QString& text ) const
{
  ...
  if( result.baseFont.pointSizeF() > 0 )  // On some rare systems, this sometimes is initialized with -1 (Qt bug?), so
  ...
  if( useBeautifulPowers )
  {
    ...
    while( result.expPart.length() > 2 &&
           result.expPart.at( 1 ) == '0' )  // length > 2 so we leave one zero when numberFormatChar is 'e'
...
QPointF QCPItemPosition::pixelPoint() const
{
  switch( mPositionType )
  {
    ...
    case ptViewportRatio:
    {
      if( mParentAnchor )
      {
        return QPointF( mKey * mParentPlot->viewport().width(), mValue * mParentPlot->viewport().height() ) +
    ...
    case ptPlotCoords:
    {
      ...
      {
        // only value axis is given, depending on orientation only transform x or y to value coordinate, other stays
        // pixel:
...
/*! \mainpage %QCustomPlot 1.2.1 Documentation
  ...
  \li Avoid repeatedly setting the complete data set with \ref QCPGraph::setData. Use \ref QCPGraph::addData instead, if
  most
      ...
      mReplotting( false )
{
  ...
  mPlotLayout->setParent( this );  // important because if parent is QWidget, QCPLayout::sizeConstraintsChanged will
...
QCPAbstractItem* QCustomPlot::itemAt( const QPointF& pos, bool onlySelectable ) const
{
  ...
  foreach( QCPAbstractItem* item, mItems )
  {
    if( onlySelectable && !item->selectable() )  // we could have also passed onlySelectable to the selectTest function,
                                 ...
                                 bool logarithmic )
{
  ...
  else  // logarithmic == true
  {
    if( mPeriodic )
    {
      ...
      {
        int index =
            (int)( qLn( data[dataIndexFactor * i] / range.lower ) / qLn( range.upper / range.lower ) * mLevelCount ) %
            mLevelCount;
        if( index < 0 )
          index += mLevelCount;
        scanLine[i] = mColorBuffer.at( index );
...
void QCPLegend::selectEvent( QMouseEvent* event, bool additive, const QVariant& details, bool* selectionStateChanged )
{
  ...
  if( details.value<SelectablePart>() == spLegendBox && mSelectableParts.testFlag( spLegendBox ) )
  {
    ...
    setSelectedParts( additive ? mSelectedParts ^ spLegendBox
                               : mSelectedParts | spLegendBox );  // no need to unset spItems in !additive case, because
...
void QCPColorScale::setType( QCPAxis::AxisType type )
{
  ...
  if( mType != type )
  {
    ...
    foreach( QCPAxis::AxisType atype, QList<QCPAxis::AxisType>() << QCPAxis::atLeft << QCPAxis::atRight
      ...
      mGradientImageInvalidated( true )
{
  ...
  foreach( QCPAxis::AxisType type, QList<QCPAxis::AxisType>() << QCPAxis::atBottom << QCPAxis::atTop << QCPAxis::atLeft
                                                              << QCPAxis::atRight )
  {
...
QCPRange QCPGraph::getKeyRange( bool& foundRange, SignDomain inSignDomain, bool includeErrors ) const
{
  ...
  else if( inSignDomain == sdNegative )  // range may only be in the negative sign domain
  {
    ...
    while( it != mData->constEnd() )
    {
      ...
      if( includeErrors )  // in case point is in valid sign domain but errobars stretch beyond it, we still want to
                           // geht that point.
      {
...
void QCPCurve::getCurveData( QVector<QPointF>* lineData ) const
{
  /* Extended sides of axis rect R divide space into 9 regions:
     ...
     2__|_R_|__8
     3  | 6 |  9
     General idea: If the two points of a line segment are in the same region (that is not R), the line segment corner
     is removed.
     Curves outside R become straight lines closely outside of R which greatly reduces drawing time, yet keeps the look
     of lines and
  ...
  */
  ...
  for( it = mData->constBegin(); it != mData->constEnd(); ++it )
  {
    ...
    */
    ...
    if( currentRegion == 5 ||
        ( firstPoint &&
          mBrush.style() != Qt::NoBrush ) )  // current is in R, add current and last if it wasn't added already
    {
      if( !addedLastAlready )  // in case curve just entered R, make sure the last point outside R is also drawn
                               // correctly
        lineData->append(
            coordsToPixels( ( it - 1 ).value().key, ( it - 1 ).value().value ) );  // add last point to vector
      else if( lastRegion != 5 )  // added last already. If that's the case, we probably added it at optimized position.
                                  // So go back and make sure it's at original position (else the angle changes under
                                  // which this segment enters R)
      {
        if( !firstPoint )  // because on firstPoint, currentRegion is 5 and addedLastAlready is true, although there is
                           // no last point
    ...
    {
      // using outsideCoordsToPixels instead of coorsToPixels for optimized point placement (places points just outside
      // axisRect instead of potentially far away)

      // if we're coming from R or we skip diagonally over the corner regions (so line might still be visible in R), we
  ...
  // If curve ends outside R, we want to add very last point so the fill looks like it should when the curve started
  // inside R:
  if( lastRegion != 5 && mBrush.style() != Qt::NoBrush && !mData->isEmpty() )
    lineData->append(
        coordsToPixels( ( mData->constEnd() - 1 ).value().key, ( mData->constEnd() - 1 ).value().value ) );
}
...
double QCPColorMapData::data( double key, double value )
{
  ...
  int valueCell =
      ( 1.0 - ( value - mValueRange.lower ) / ( mValueRange.upper - mValueRange.lower ) ) * ( mValueSize - 1 ) + 0.5;
  ...
  if( keyAxis->orientation() == Qt::Horizontal )
  {
    ...
    for( int line = 0; line < lineCount; ++line )
    {
      QRgb* pixels = reinterpret_cast<QRgb*>(
          mMapImage.scanLine( lineCount - 1 - line ) );  // invert scanline index because QImage counts scanlines from
...
double QCPItemTracer::selectTest( const QPointF& pos, bool onlySelectable, QVariant* details ) const
{
  ...
  switch( mStyle )
  {
    ...
    {
      return qSqrt(
          qMin( distSqrToLine( QPointF( clip.left(), center.y() ), QPointF( clip.right(), center.y() ), pos ),
