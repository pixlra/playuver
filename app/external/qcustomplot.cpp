...
{
#if QT_VERSION < QT_VERSION_CHECK( 5, 0, 0 )  // before Qt5, default pens used to
                                              // be cosmetic if
...
QCPLayer::QCPLayer( QCustomPlot* parentPlot, const QString& layerName )
    ...
    , mName( layerName )
    , mIndex( -1 )
    ,  // will be set to a proper value by the QCustomPlot layer
       // creation function
    mVisible( true )
{
...
/*!
  ...
  or QCPItemText, a
  click inside the area returns a constant value greater zero (typically the
  selectionTolerance of
  the parent QCustomPlot multiplied by 0.99). If the click lies outside the
  area, this function
  returns -1.0.
...
/*! \page thelayoutsystem The Layout System
  ...
  All objects that
  take part in the layout system derive from this class, either directly or
  indirectly.

  Since QCPLayoutElement itself derives from \ref QCPLayerable, a layout element
  may draw its own
  content. However, it is perfectly possible for a layout element to only serve
  as a structuring
  ...
  rect (\ref
  QCPLayoutElement::rect) and the outer rect (\ref
  QCPLayoutElement::setOuterRect). The inner rect
  is calculated automatically by applying the margin (\ref
  QCPLayoutElement::setMargins) inward
  from the outer rect. The inner rect is meant for main content while the margin
  area may either be
  left blank or serve for displaying peripheral graphics. For example, \ref
  QCPAxisRect positions
  the four main axes at the sides of the inner rect, so graphs end up inside it
  and the axis labels
  ...
  margins. Internally,
  this is realized with the \ref QCPLayoutElement::calculateAutoMargin function
  which takes a \ref
  ...
  QCPLayoutElement::setAutoMargins. By default, it is set to \ref QCP::msAll
  meaning automatic
  margin calculation is enabled for all four sides. In this case, a minimum
  margin may be set with
  \ref QCPLayoutElement::setMinimumMargins, to prevent the automatic margin
  mechanism from setting
  margins smaller than desired for a specific situation. If automatic margin
  calculation is unset
  for a specific side, the margin of that side can be controlled directy via
  \ref
  QCPLayoutElement::setMargins.

  If multiple layout ements are arranged next to or beneath each other, it may
  be desirable to
  align their inner rects on certain sides. Since they all might have different
  automatic margins,
  this usually isn't the case. The class \ref QCPMarginGroup and \ref
  QCPLayoutElement::setMarginGroup fix this by allowing to synchronize multiple
  margins. See the
  ...
  elements and in
  princple can have the only purpose to manage/arrange those child elements.
  This is what the
  subclass \ref QCPLayout specializes on. It is a QCPLayoutElement itself but
  has no visual
  representation. It defines an interface to add, remove and manage child layout
  elements.
  QCPLayout isn't a usable layout though, it's an abstract base class that
  concrete layouts derive
  ...
  It shows how two child layout elements are placed inside the grid layout next
  to each other in
  cells (0, 0) and (0, 1).
  ...
  accessible via \ref
  QCustomPlot::plotLayout and contains (directly or indirectly via other
  sub-layouts) all layout
  elements in the QCustomPlot. By default, this top level grid layout contains a
  single cell which
  ...
  rect
  customPlot->plotLayout()->addElement(0, 0, title); // place the title in the
  empty cell we've just created
  \endcode
  ...
  layout system.
  \code
  customPlot->plotLayout()->clear(); // let's start from scratch and remove the
  default axis rect
  ...
  allAxes << topAxisRect->axes() << leftAxisRect->axes() <<
  rightAxisRect->axes();
...
QCPLayoutElement::QCPLayoutElement( QCustomPlot* parentPlot )
    ...
    ,  // parenthood is changed as soon as layout
       // element gets inserted into a layout
       // (except for top level layout)
    mParentLayout( 0 )
    , mMinimumSize()
...
void QCPLayoutElement::update( UpdatePhase phase )
{
  if( phase == upMargins )
  {
    if( mAutoMargins != QCP::msNone )
    {
      ...
      // calculation, either directly or via a
      // margin group:
...
QVector<int> QCPLayout::getSectionSizes( QVector<int> maxSizes, QVector<int> minSizes, QVector<double> stretchFactors, int totalSize ) const
{
  ...
  while( !unfinishedSections.isEmpty() && outerIterations < sectionCount * 2 )  // the iteration check ist just a
                                                                                // failsafe in case something
                                                                                // really strange happens
  {
    ++outerIterations;
    int innerIterations = 0;
    while( !unfinishedSections.isEmpty() && innerIterations < sectionCount * 2 )  // the iteration check ist just a
                                                                                  // failsafe in case something
                                                                                  // really strange happens
    {
      ++innerIterations;
      ...
      // (i.e. can we stretch all remaining
...
void QCPLayoutInset::updateLayout()
{
  for( int i = 0; i < mElements.size(); ++i )
  {
    ...
    finalMaxSize.setWidth( mElements.at( i )->maximumSize().width() < QWIDGETSIZE_MAX ? mElements.at( i )->maximumSize().width()
                                                                                      : maxSizeHint.width() );
    finalMaxSize.setHeight( mElements.at( i )->maximumSize().height() < QWIDGETSIZE_MAX ? mElements.at( i )->maximumSize().height()
                                                                                        : maxSizeHint.height() );
    if( mInsetPlacement.at( i ) == ipFree )
    {
      insetRect = QRect( rect().x() + rect().width() * mInsetRect.at( i ).x(), rect().y() + rect().height() * mInsetRect.at( i ).y(),
...
void QCPLineEnding::draw( QCPPainter* painter, const QVector2D& pos, const QVector2D& dir ) const
{
  ...
  case esLineArrow:
  {
    QPointF points[3] = {( pos - lengthVec + widthVec ).toPointF(), pos.toPointF(), ( pos - lengthVec - widthVec ).toPointF()};
    painter->setPen( miterPen );
  ...
  {
    QVector2D widthVecPerp( -widthVec.y(), widthVec.x() );
    QPointF points[4] = {( pos - widthVecPerp ).toPointF(), ( pos - widthVec ).toPointF(), ( pos + widthVecPerp ).toPointF(),
  ...
  case esSkewedBar:
  {
    ...
    else
    {
      ...
      painter->drawLine(
          ( pos + widthVec + lengthVec * 0.2f * ( mInverted ? -1 : 1 ) + dir.normalized() * qMax( 1.0f, (float)painter->pen().widthF() ) * 0.5f )
              .toPointF(),
          ( pos - widthVec - lengthVec * 0.2f * ( mInverted ? -1 : 1 ) + dir.normalized() * qMax( 1.0f, (float)painter->pen().widthF() ) * 0.5f )
              .toPointF() );
    }
...
void QCPAxis::rescale( bool onlyVisiblePlottables )
{
  ...
  if( haveRange )
  {
    ...
    {
      double center = ( newRange.lower + newRange.upper ) * 0.5;  // upper and lower should be equal anyway, but just to
                                                                  // make sure, incase validRange returned false for
                                                                  // other reason
      if( mScaleType == stLinear )
      {
  ...
  if( mAutoTickLabels )
  {
    ...
    else if( mTickLabelType == ltDateTime )
    {
      for( int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i )
      {
        ...
        mTickVectorLabels[i] =
            mParentPlot->locale().toString( QDateTime::fromTime_t( mTickVector.at( i ) ).toTimeSpec( mDateTimeSpec ), mDateTimeFormat );
#else
...
void QCPAxisPainterPrivate::draw( QCPPainter* painter )
{
  ...
  if( !tickPositions.isEmpty() )
  {
    ...
    int tickDir = ( type == QCPAxis::atBottom || type == QCPAxis::atRight ) ? -1 : 1;  // direction of ticks ("inward" is right for left
  ...
  if( lowerEnding.style() != QCPLineEnding::esNone )
    lowerEnding.draw( painter,
                      QVector2D( baseLine.p1() ) - baseLineVector.normalized() * lowerEnding.realLength() * ( lowerEnding.inverted() ? -1 : 1 ),
                      -baseLineVector );
  if( upperEnding.style() != QCPLineEnding::esNone )
    upperEnding.draw( painter,
                      QVector2D( baseLine.p2() ) + baseLineVector.normalized() * upperEnding.realLength() * ( upperEnding.inverted() ? -1 : 1 ),
                      baseLineVector );
  ...
  if( !label.isEmpty() )
  {
    ...
    else if( type == QCPAxis::atRight )
    {
      ...
      painter->drawText( origin.x(), origin.y() - margin - labelBounds.height(), alignmentRect.width(), labelBounds.height(),
  ...
  if( type == QCPAxis::atLeft )
  {
    ...
    mTickLabelsSelectionBox.setCoords( origin.x() - selTickLabelOffset - selTickLabelSize, alignmentRect.top(), origin.x() - selTickLabelOffset,
                                       alignmentRect.bottom() );
    mLabelSelectionBox.setCoords( origin.x() - selLabelOffset - selLabelSize, alignmentRect.top(), origin.x() - selLabelOffset,
                                  alignmentRect.bottom() );
  }
  else if( type == QCPAxis::atRight )
  {
  ...
  else if( type == QCPAxis::atTop )
  {
    ...
    mTickLabelsSelectionBox.setCoords( alignmentRect.left(), origin.y() - selTickLabelOffset - selTickLabelSize, alignmentRect.right(),
                                       origin.y() - selTickLabelOffset );
    mLabelSelectionBox.setCoords( alignmentRect.left(), origin.y() - selLabelOffset - selLabelSize, alignmentRect.right(),
...
QCPAxisPainterPrivate::TickLabelData QCPAxisPainterPrivate::getTickLabelData( const QFont& font, const QString& text ) const
{
  ...
  if( result.baseFont.pointSizeF() > 0 )  // On some rare systems, this sometimes
                                          // is initialized with -1 (Qt bug?), so
    // we check here before possibly setting a negative value in the next line
  ...
  result.totalBounds.moveTopLeft( QPoint( 0, 0 ) );  // want bounding box aligned top left at origin, independent of how
                                                     // it was created, to make further processing simpler
...
QPointF QCPAxisPainterPrivate::getTickLabelDrawOffset( const TickLabelData& labelData ) const
{
  ...
  */
  ...
  bool flip = qFuzzyCompare( qAbs( tickLabelRotation ), 90.0 );  // perfect +/-90 degree flip. Indicates
...
/*! \class QCPAbstractPlottable
  ...
  inferred from the axes that are passed in the
  constructor.</td>
  </tr><tr>
  ...
  prominent data representing lines in the
  ...
  (hint: \ref mainPen gives you the right pen,
  ...
  most prominent fillable structures in the
  plottable (e.g. QCPGraph uses this brush to control filling under the
  graph)</td>
  </tr><tr>
    <td>QBrush \b mSelectedBrush</td>
    <td>The generic brush that should be used when the plottable is selected
  (hint: \ref mainBrush gives you the right
  ...
  QCPAxis::coordToPixel functions to translate
  ...
  of the axes is null, don't draw the
  plottable.</td>
  </tr><tr>
...
QCPItemAnchor::~QCPItemAnchor()
{
  ...
  for( int i = 0; i < currentChildren.size(); ++i )
    currentChildren.at( i )->setParentAnchor( 0 );  // this acts back on this anchor
                                                    // and child removes itself from
                                                    // mChildren
}
...
/*! \class QCPItemPosition
  ...
  setType. This type defines
  how coordinates passed to \ref setCoords are to be interpreted, e.g. as
  absolute pixel coordinates, as
  plot coordinates of certain axes, etc.

  Further, QCPItemPosition may have a parent QCPItemAnchor, see \ref
  setParentAnchor. (Note that every
  QCPItemPosition inherits from QCPItemAnchor and thus can itself be used as
  parent anchor for other
  positions.) This way you can tie multiple items together. If the
  QCPItemPosition has a parent, the
  coordinates set with \ref setCoords are considered to be absolute values in
  the reference frame of the
  parent anchor, where (0, 0) means directly ontop of the parent anchor. For
  example, You could attach
  the \a start position of a QCPItemLine to the \a bottom anchor of a
  QCPItemText to make the starting
  point of the line always be centered under the text label, no matter where the
  text is moved to, or is
  itself tied to.
...
/*!
  ...
  anchor always is absolute with (0, 0) being exactly on top of the parent
  anchor. (Hence the type
...
QPointF QCPItemPosition::pixelPoint() const
{
  ...
  case ptViewportRatio:
  {
    if( mParentAnchor )
    {
      return QPointF( mKey * mParentPlot->viewport().width(), mValue * mParentPlot->viewport().height() ) + mParentAnchor->pixelPoint();
    }
    else
    {
      return QPointF( mKey * mParentPlot->viewport().width(), mValue * mParentPlot->viewport().height() ) + mParentPlot->viewport().topLeft();
    }
  }
  ...
  case ptAxisRectRatio:
  {
    if( mAxisRect )
    {
      if( mParentAnchor )
      {
        return QPointF( mKey * mAxisRect.data()->width(), mValue * mAxisRect.data()->height() ) + mParentAnchor->pixelPoint();
      }
      else
      {
        return QPointF( mKey * mAxisRect.data()->width(), mValue * mAxisRect.data()->height() ) + mAxisRect.data()->topLeft();
      }
    }
    else
    {
...
/*! \class QCPAbstractItem
  ...
  sticks to its data points, given a key
  coordinate.</td></tr>
  </table>
...
/*! \internal
  ...
  all four returned
  values which were greater or equal to zero. (Because this function may return
  -1.0 when \a pos
  doesn't hit \a rect at all). If all calls returned -1.0, return -1.0, too,
  because your item
  wasn't hit.
...
double QCPAbstractItem::rectSelectTest( const QRectF& rect, const QPointF& pos, bool filledRect ) const
{
  ...
  // distance to border:
  QList<QLineF> lines;
  lines << QLineF( rect.topLeft(), rect.topRight() ) << QLineF( rect.bottomLeft(), rect.bottomRight() ) << QLineF( rect.topLeft(), rect.bottomLeft() )
...
/*! \mainpage %QCustomPlot 1.2.1 Documentation
  ...
  you are new to
  QCustomPlot and just want to start using it, it's recommended to look at the
  tutorials and
  examples at
  ...
  with the basic
  concept of how to use %QCustomPlot and you wish to learn more about specific
  functionality.
  See the \ref classoverview "class overview" for diagrams explaining the
  ...
  QCustomPlot. Every
  QCustomPlot contains four axes by default. They can be accessed via the
  members \ref
  QCustomPlot::xAxis "xAxis", \ref QCustomPlot::yAxis "yAxis", \ref
  QCustomPlot::xAxis2 "xAxis2"
  and \ref QCustomPlot::yAxis2 "yAxis2", and are of type QCPAxis. QCustomPlot
  supports an arbitrary
  number of axes and axis rects, see the documentation of QCPAxisRect for
  details.

  \section mainpage-plottables Plottables

  \a Plottables are classes that display any kind of data inside the
  QCustomPlot. They all derive
  from QCPAbstractPlottable. For example, the QCPGraph class is a plottable that
  displays a graph
  ...
  You create a new graph with QCustomPlot::addGraph and access them with
  QCustomPlot::graph.
  ...
  of that function is the abstract base class of all plottables,
  QCPAbstractPlottable, you will
  probably want to qobject_cast the returned pointer to the respective plottable
  subclass. (As
  usual, if the cast returns zero, the plottable wasn't of that specific
  subclass.)

  All further interfacing with plottables (e.g how to set data) is specific to
  the plottable type.
  See the documentations of the subclasses: QCPGraph, QCPCurve, QCPBars,
  QCPStatisticalBox,
  QCPColorMap.
  ...
  with the
  QCPAxis::setRange function. By default, the axes represent a linear scale. To
  set a logarithmic
  scale, set \ref QCPAxis::setScaleType to \ref QCPAxis::stLogarithmic. The
  logarithm base can be set freely
  ...
  QCPAxis::setAutoTicks,
  QCPAxis::setAutoTickCount, QCPAxis::setAutoTickStep, QCPAxis::setTickLabels,
  QCPAxis::setTickLabelType, QCPAxis::setTickLabelRotation,
  QCPAxis::setTickStep,
  ...
  \ref QCPAxisRect::setAutoMargins to exclude the respective margin sides, set
  the margins manually with
  \ref QCPAxisRect::setMargins. The main axis rect can be reached with \ref
  ...
  A legend is a small
  layout element inside the plot which lists the plottables with an icon of the
  plottable
  line/symbol and a name (QCPAbstractPlottable::setName). Plottables can be
  ...
  QCustomPlot
  automatically adds it to the legend, too. This behaviour can be modified with
  the
  ...
  directly, too. See
  QCPLegend::item, QCPLegend::itemWithPlottable, QCPLegend::addItem,
  QCPLegend::removeItem for
  example.
  ...
  QCustomPlot::setInteractions. For
  ...
  clicked or
  doubleClicked. See \ref QCustomPlot::plottableClick, \ref
  ...
  important: Items. The
  base class of all items is QCPAbstractItem. An item sets itself apart from
  plottables in that
  it's not necessarily bound to any axes. This means it may also be positioned
  in absolute pixel
  coordinates or placed at a relative position on an axis rect. Further, it
  usually doesn't
  ...
  example, you could place the head of an arrow at a fixed plot coordinate, so
  it always points to
  some important area in the plot. The tail of the arrow can be anchored to a
  text item which
  always resides in the top center of the axis rect, independent of where the
  user drags the axis
  ...
  positioning of objects like
  the axis rect(s), legends and the plot title. They are all based on \ref
  QCPLayoutElement and are arranged by
  ...
  antialiasing and thick
  lines can cause a significant slow down. If you notice this in your
  application, here are some
  thoughts on how to increase performance. By far the most time is spent in the
  drawing functions,
  specifically the drawing of graphs. For maximum performance, consider the
  following (most
  recommended/effective measures first):

  \li use Qt 4.8.0 and up. Performance has doubled or tripled with respect to Qt
  4.7.4. However
  QPainter was broken and drawing pixel precise things, e.g. scatters, isn't
  possible with Qt >=
  4.8.0. So it's a performance vs. plot quality tradeoff when switching to Qt
  4.8.
  \li To increase responsiveness during dragging, consider setting \ref
  QCustomPlot::setNoAntialiasingOnDrag to true.
  \li On X11 (GNU/Linux), avoid the slow native drawing system, use raster by
  supplying
  "-graphicssystem raster" as command line argument or calling
  QApplication::setGraphicsSystem("raster")
  before creating the QApplication object. (Only available for Qt versions
  before 5.0)
  \li On all operating systems, use OpenGL hardware acceleration by supplying
  "-graphicssystem
  opengl" as command line argument or calling
  QApplication::setGraphicsSystem("opengl") (Only
  available for Qt versions before 5.0). If OpenGL is available, this will
  slightly decrease the
  quality of antialiasing, but extremely increase performance especially with
  alpha
  (semi-transparent) fills, much antialiasing and a large QCustomPlot drawing
  surface. Note
  however, that the maximum frame rate might be constrained by the vertical sync
  frequency of your
  monitor (VSync can be disabled in the graphics card driver configuration). So
  for simple plots
  (where the potential framerate is far above 60 frames per second), OpenGL
  acceleration might
  achieve numerically lower frame rates than the other graphics systems, because
  they are not
  ...
  QCustomPlot::setNotAntialiasedElements)
  \li Avoid repeatedly setting the complete data set with \ref
  QCPGraph::setData. Use \ref QCPGraph::addData instead, if
  most
  ...
  pointers get
  transferred. (Relevant only if preparing data maps with a large number of
  points, i.e. over 10000)
  ...
  <dd>Define this flag when you compile QCustomPlot as a shared library
  (.so/.dll)
  <dt>\c QCUSTOMPLOT_USE_LIBRARY
  <dd>Define this flag before including the header, when using QCustomPlot as a
  shared library
  <dt>\c QCUSTOMPLOT_CHECK_DATA
  <dd>If this flag is defined, the QCustomPlot plottables will perform data
  validity checks on every redraw.
      This means they will give qDebug output when you plot \e inf or \e nan
  values, they will not
      fix your data.
  </dl>
...
/*! \page classoverview Class Overview
  ...
  relationships between classes that make up
  the QCustomPlot library. The diagrams are not exhaustive, so only the classes
  deemed most relevant are shown.
  ...
  \image html InheritanceOverview.png "Inheritance tree of most important
  classes"
...
QCPAbstractPlottable* QCustomPlot::plottableAt( const QPointF& pos, bool onlySelectable ) const
{
  ...
  double resultDistance = mSelectionTolerance;  // only regard clicks with
                                                // distances smaller than
                                                // mSelectionTolerance as
...
QCPAbstractItem* QCustomPlot::itemAt( const QPointF& pos, bool onlySelectable ) const
{
  ...
  foreach( QCPAbstractItem* item, mItems )
  {
    if( onlySelectable && !item->selectable() )  // we could have also passed
      ...
      continue;
    if( !item->clipToAxisRect() || item->clipRect().contains( pos.toPoint() ) )  // only consider clicks inside
...
void QCustomPlot::replot( QCustomPlot::RefreshPriority refreshPriority )
{
  ...
  if( painter.isActive() )
  {
    painter.setRenderHint( QPainter::HighQualityAntialiasing );  // to make Antialiasing look good if
...
bool QCustomPlot::savePdf( const QString& fileName, bool noCosmeticPen, int width, int height, const QString& pdfCreator, const QString& pdfTitle )
{
  ...
  if( printpainter.begin( &printer ) )
  {
    ...
    if( mBackgroundBrush.style() != Qt::NoBrush && mBackgroundBrush.color() != Qt::white && mBackgroundBrush.color() != Qt::transparent &&
...
void QCustomPlot::mouseDoubleClickEvent( QMouseEvent* event )
{
  ...
  // since the mouseDoubleClick replaces the
...
QPixmap QCustomPlot::toPixmap( int width, int height, double scale )
{
  ...
  // change in toPainter might be necessary,
  // too.
  int newWidth, newHeight;
  if( width == 0 || height == 0 )
  {
  ...
  result.fill( mBackgroundBrush.style() == Qt::SolidPattern ? mBackgroundBrush.color() : Qt::transparent );  // if using non-solid pattern, make
  ...
  if( painter.isActive() )
  {
    ...
    if( !qFuzzyCompare( scale, 1.0 ) )
    {
      if( scale > 1.0 )  // for scale < 1 we always want cosmetic pens where
                         // possible, because else lines might
...
/*!
  ...
  create a temporary QPicture and open a
  QCPPainter
...
void QCustomPlot::toPainter( QCPPainter* painter, int width, int height )
{
  ...
  // change in toPixmap might be necessary,
  // too.
  int newWidth, newHeight;
  if( width == 0 || height == 0 )
  {
  ...
  if( painter->isActive() )
  {
    ...
    // background color is applied there via
    // QPixmap::fill
...
/*! \class QCPColorGradient
  ...
  color. For example,
  QCPColorMap and QCPColorScale have a \ref QCPColorMap::setGradient
  "setGradient" method which
  takes an instance of this class. Colors are set with \ref
  ...
  if( mColorStops.size() > 1 )
  {
    ...
    for( int i = 0; i < mLevelCount; ++i )
    {
            ...
            // interpolate color
      {
        ...
        case ciRGB:
        {
          mColorBuffer[i] = qRgb( ( 1 - t ) * low.value().red() + t * high.value().red(), ( 1 - t ) * low.value().green() + t * high.value().green(),
...
/*!
  ...
  endings of the new
  axis (\ref QCPAxis::setLowerEnding, \ref QCPAxis::setUpperEnding) are
  initialized to \ref
...
bool QCPAxisRect::removeAxis( QCPAxis* axis )
{
  ...
  // pointer, rather go through all axis
  // containers:
  QHashIterator<QCPAxis::AxisType, QList<QCPAxis*>> it( mAxes );
  while( it.hasNext() )
  {
    ...
    if( it.value().contains( axis ) )
    {
      ...
      if( qobject_cast<QCustomPlot*>( parentPlot() ) )  // make sure this isn't
                                                        // called from QObject dtor
                                                        // when
        // QCustomPlot is already destructed (happens when the axis rect
        // is not in any layout and thus QObject-child of QCustomPlot)
...
void QCPAxisRect::update( UpdatePhase phase )
{
  ...
  // QCPAxisRect doesn't derive from
  // QCPLayout):
...
void QCPAxisRect::updateAxesOffset( QCPAxis::AxisType type )
{
  ...
  bool isFirstVisible = !axesList.first()->visible();  // if the first axis is
                                                       // visible, the second
                                                       // axis (which is where
...
void QCPAxisRect::mousePressEvent( QMouseEvent* event )
{
  mDragStart = event->pos();  // need this even when not LeftButton is pressed,
                              // to determine in releaseEvent whether it
...
/*! \class QCPAbstractLegendItem
  ...
  least the most prominent text of the
  item.</td>
    </tr>
  </table>
*/
...
void QCPPlottableLegendItem::draw( QCPPainter* painter )
{
  ...
  int textHeight = qMax( textRect.height(), iconSize.height() );  // if text has smaller height than
...
/*!
  ...
  associated with \a plottable (e.g. a \ref
  QCPGraph*).
...
void QCPLegend::selectEvent( QMouseEvent* event, bool additive, const QVariant& details, bool* selectionStateChanged )
{
  ...
  {
    SelectableParts selBefore = mSelectedParts;
    setSelectedParts( additive ? mSelectedParts ^ spLegendBox : mSelectedParts | spLegendBox );  // no need to
                                                                                                 // unset spItems
                                                                                                 // in !additive
                                                                                                 // case, because
    // they will be deselected by QCustomPlot (they're
    // normal QCPLayerables with own deselectEvent)
    if( selectionStateChanged )
      *selectionStateChanged = mSelectedParts != selBefore;
  }
}
...
/*! \class QCPPlotTitle
  ...
  default axis rect
  customPlot->plotLayout()->addElement(0, 0, new QCPPlotTitle(customPlot, "Your
  Plot Title"));
  \endcode
...
QCPColorScale::QCPColorScale( QCustomPlot* parentPlot )
    : QCPLayoutElement( parentPlot )
    , mType( QCPAxis::atTop )
    ,  // set to atTop such that setType(QCPAxis::atRight)
       // below doesn't skip work because it
...
void QCPColorScale::setType( QCPAxis::AxisType type )
{
  ...
  if( mType != type )
  {
    ...
    mColorAxis.data()->setScaleLogBase( logBaseTransfer );  // scaleType is
                                                            // synchronized among
                                                            // axes in realtime via
...
void QCPColorScale::rescaleDataRange( bool onlyVisibleMaps )
{
  ...
  int sign = 0;  // TODO: should change this to QCPAbstractPlottable::SignDomain
                 // later (currently is protected, maybe
  ...
  case upMargins:
  {
    if( mType == QCPAxis::atBottom || mType == QCPAxis::atTop )
    {
      setMaximumSize( QWIDGETSIZE_MAX,
                      mBarWidth + mAxisRect.data()->margins().top() + mAxisRect.data()->margins().bottom() + margins().top() + margins().bottom() );
      setMinimumSize( 0,
                      mBarWidth + mAxisRect.data()->margins().top() + mAxisRect.data()->margins().bottom() + margins().top() + margins().bottom() );
    }
    else
    {
      setMaximumSize( mBarWidth + mAxisRect.data()->margins().left() + mAxisRect.data()->margins().right() + margins().left() + margins().right(),
                      QWIDGETSIZE_MAX );
      setMinimumSize( mBarWidth + mAxisRect.data()->margins().left() + mAxisRect.data()->margins().right() + margins().left() + margins().right(),
                      0 );
    }
    break;
  }
...
/*!
  ...
  keyErrorMinus and \a valueErrorMinus. The
  positive
...
/*!
  ...
  QCPScatterStyle::ssNone, no scatter
  points
...
// check data validity if flag set:
  ...
  {
    if( QCP::isInvalidData( it.value().key, it.value().value ) || QCP::isInvalidData( it.value().keyErrorPlus, it.value().keyErrorMinus ) ||
...
void QCPGraph::getPreparedData( QVector<QCPData>* lineData, QVector<QCPData>* scatterData ) const
{
  ...
  // count to the limit maxCount if using
  // adaptive sampling:
                                                    ...
                                                    // points per pixel on average
  {
    if( lineData )
    {
      ...
      int reversedFactor = keyAxis->rangeReversed() ? -1 : 1;  // is used to calculate keyEpsilon
                                                               // pixel into the correct direction
      int reversedRound = keyAxis->rangeReversed() ? 1 : 0;    // is used to switch between floor
      ...
      double keyEpsilon = qAbs( currentIntervalStartKey - keyAxis->pixelToCoord( keyAxis->coordToPixel( currentIntervalStartKey ) +
                                                                                 1.0 * reversedFactor ) );  // interval of one pixel
                                                                                                            // on screen when mapped
                                                                                                            // to plot key
                                                                                                            // coordinates
      bool keyEpsilonVariable = keyAxis->scaleType() == QCPAxis::stLogarithmic;                             // indicates whether keyEpsilon needs
      ...
      while( it != upperEnd )
      {
        if( it.key() < currentIntervalStartKey + keyEpsilon )  // data point is
                                                               // still within
                                                               // same pixel, so
                                                               // skip it and
        ...
        else  // new pixel interval started
        {
          ...
          {
            if( lastIntervalEndKey < currentIntervalStartKey - keyEpsilon )  // last point is further away, so first
                                                                             // point of this cluster must be at a real
                                                                             // data point
            ...
            if( it.key() > currentIntervalStartKey + keyEpsilon * 2 )  // new pixel started further away from previous
          ...
          if( keyEpsilonVariable )
            keyEpsilon =
                qAbs( currentIntervalStartKey - keyAxis->pixelToCoord( keyAxis->coordToPixel( currentIntervalStartKey ) + 1.0 * reversedFactor ) );
          intervalDataCount = 1;
        }
        ++it;
      }
      ...
      if( intervalDataCount >= 2 )  // last pixel had multiple data points,
                                    // consolidate them to a cluster
      {
        if( lastIntervalEndKey < currentIntervalStartKey - keyEpsilon )  // last point wasn't a cluster, so first point
    ...
    if( scatterData )
    {
      ...
      while( it != upperEnd )
      {
        ...
        else  // new pixel started
        {
          if( intervalDataCount >= 2 )  // last pixel had multiple data points, consolidate them
          {
            ...
            // maintain certain vertical data density
            ...
            int dataModulo = qMax( 1,
                                   qRound( intervalDataCount / ( valuePixelSpan / 4.0 ) ) );  // approximately every 4
            ...
            while( intervalIt != it )
            {
              if( ( c % dataModulo == 0 || intervalIt == minValueIt || intervalIt == maxValueIt ) && intervalIt.value().value > valueMinRange &&
      ...
      {
        // determine value pixel span and add as many points in interval to
        // maintain certain vertical data density (this
        // is specific to scatter plot):
        double valuePixelSpan = qAbs( valueAxis->coordToPixel( minValue ) - valueAxis->coordToPixel( maxValue ) );
        int dataModulo = qMax( 1, qRound( intervalDataCount / ( valuePixelSpan / 4.0 ) ) );  // approximately every 4
                                                                                             // value pixels one data
                                                                                             // point on average
        QCPDataMap::const_iterator intervalIt = currentIntervalStart;
        int c = 0;
  ...
  else  // valueAxis->mScaleType == QCPAxis::stLogarithmic
  {
    ...
    if( keyAxis->orientation() == Qt::Vertical )
    {
      if( ( valueAxis->range().upper < 0 && !valueAxis->rangeReversed() ) ||
          ( valueAxis->range().upper > 0 && valueAxis->rangeReversed() ) )  // if range is negative, zero is on
...
const QPolygonF QCPGraph::getChannelFillPolygon( const QVector<QPointF>* lineData ) const
{
  ...
  if( mChannelFillGraph.data()->mKeyAxis.data()->orientation() != keyAxis->orientation() )
    return QPolygonF();  // don't have same axis orientation, can't fill that
                         // (Note: if keyAxis fits, valueAxis will fit
  ...
  if( keyAxis->orientation() == Qt::Horizontal )
  {
    ...
    // Reverse them, since following algorithm
...
double QCPGraph::pointDistance( const QPointF& pixelPoint ) const
{
  ...
  if( mLineStyle == lsNone )
  {
    ...
    QPointF ptB = coordsToPixels( scatterData->at( 0 ).key,
                                  scatterData->at( 0 ).value );  // getScatterPlotData returns
                                                                 // in plot coordinates, so
                                                                 // transform to pixels
...
QCPRange QCPGraph::getKeyRange( bool& foundRange, SignDomain inSignDomain, bool includeErrors ) const
{
  ...
  else if( inSignDomain == sdNegative )  // range may only be in the negative sign domain
  {
    ...
    while( it != mData->constEnd() )
    {
      ...
      if( includeErrors )  // in case point is in valid sign domain but errobars
                           // stretch beyond it, we still want to
                           // geht that point.
      {
  ...
  else if( inSignDomain == sdPositive )  // range may only be in the positive sign domain
  {
    ...
    while( it != mData->constEnd() )
    {
      ...
      if( includeErrors )  // in case point is in valid sign domain but errobars
                           // stretch beyond it, we still want to get
                           // that point.
      {
        if( ( current < range.lower || !haveLower ) && current > 0 )
        {
...
/*! \internal
  ...
  the line of the
  curve. Line segments that aren't visible in the current axis rect are handled
  in an optimized
  way.
*/
void QCPCurve::getCurveData( QVector<QPointF>* lineData ) const
{
  /* Extended sides of axis rect R divide space into 9 regions:
     ...
     (that is not R), the line segment corner
     is removed.
     Curves outside R become straight lines closely outside of R which greatly
     reduces drawing time, yet keeps the look
     of lines and
  ...
  */
  ...
  for( it = mData->constBegin(); it != mData->constEnd(); ++it )
  {
    ...
    /*
      ...
      it seems like the
      whole thing is still drawn, but actually the points outside the axisRect
      are simplified
      ("optimized") greatly. There are some subtle special cases when line
      segments are large and
      thereby each subsequent point may be in a different region or even skip
      some.
    */
    // determine whether to keep current point:
    if( currentRegion == 5 || ( firstPoint && mBrush.style() != Qt::NoBrush ) )  // current is in R, add
                                                                                 // current and last if it
                                                                                 // wasn't added already
    {
      if( !addedLastAlready )  // in case curve just entered R, make sure the last
                               // point outside R is also drawn
                               // correctly
        lineData->append( coordsToPixels( ( it - 1 ).value().key,
                                          ( it - 1 ).value().value ) );  // add last point to vector
      else if( lastRegion != 5 )                                         // added last already. If that's the case, we
                                                                         // probably added it at optimized position.
      // So go back and make sure it's at original position (else the angle
      // changes under
      // which this segment enters R)
      {
        if( !firstPoint )  // because on firstPoint, currentRegion is 5 and
                           // addedLastAlready is true, although there is
                           // no last point
                                            ...
                                            // last if not added already
    {
      ...
      // point placement (places points just outside
      // axisRect instead of potentially far away)

      // if we're coming from R or we skip diagonally over the corner regions
      // (so line might still be visible in R), we
      // can't place points optimized
      ...
      addedLastAlready = true;  // so that if next point enters 5, or crosses
                                // another region boundary, we don't add this
                                // point twice
    }
  ...
  // like it should when the curve started
  // inside R:
...
/*! \internal
  ...
  the visible
  axisRect and just crossing a boundary (since \ref getCurveData reduces
  non-visible curve segments
  to those line segments that cross region boundaries, see documentation there).
  It only uses the
  coordinate parallel to the region boundary of the axisRect. The other
  coordinate is picked just
  outside the axisRect (how far is determined by the scatter size and the line
  width). Together
  with the optimization in \ref getCurveData this improves performance for large
  curves (or zoomed
  in ones) significantly while keeping the illusion the whole curve and its
  filling is still being
...
/*! \class QCPBars
  ...
  top of each other
  (see \ref QCPBars::moveAbove). Then, when two bars are at the same key
  position, they will appear
  stacked.
...
/*! \internal
  ...
  0-value upwards,
  negative are stacked below 0-value downwards). This can be indicated with \a
  positive. So if the
...
/*! \internal
  ...
  properties.
  The bar(s) currently below lower and upper will become disconnected to
  lower/upper.
...
/*!
  ...
  other.
*/
QCPColorMapData::QCPColorMapData( const QCPColorMapData& other ) : mKeySize( 0 ), mValueSize( 0 ), mIsEmpty( true ), mData( 0 ), mDataModified( true )
{
  *this = other;
}
...
#ifdef __EXCEPTIONS
      try
      {  // 2D arrays get memory intensive fast. So if the allocation fails,
...
void QCPColorMap::updateMapImage()
{
  ...
  if( keyAxis->orientation() == Qt::Horizontal )
  {
    ...
    {
      QRgb* pixels =
          reinterpret_cast<QRgb*>( mMapImage.scanLine( lineCount - 1 - line ) );  // invert scanline index because QImage counts scanlines from
...
/*! \class QCPItemStraightLine
  ...
  circles are anchors, solid blue discs are
  positions."
...
QLineF QCPItemStraightLine::getRectClippedStraightLine( const QVector2D& base, const QVector2D& vec, const QRect& rect ) const
{
  ...
  else  // line is skewed
  {
    ...
    else if( pointVectors.size() > 2 )
    {
      ...
      // single out the point pair with greatest
      // distance:
      double distSqrMax = 0;
      QVector2D pv1, pv2;
...
/*!
  ...
  Qt::AlignLeft, Qt::AlignCenter or
  Qt::AlignRight).
*/
...
/*!
  ...
  The constructed item can be added to the plot with QCustomPlot::addItem.
*/
QCPItemTracer::QCPItemTracer( QCustomPlot* parentPlot ) : QCPAbstractItem( parentPlot ), position( createPosition( "position" ) ), mGraph( 0 )
{
  position->setCoords( 0, 0 );
...
void QCPItemBracket::draw( QCPPainter* painter )
{
  ...
  boundingPoly << leftVec.toPoint() << rightVec.toPoint() << ( rightVec - lengthVec ).toPoint() << ( leftVec - lengthVec ).toPoint();
  QRect clip = clipRect().adjusted( -mainPen().widthF(), -mainPen().widthF(), mainPen().widthF(), mainPen().widthF() );
  if( clip.intersects( boundingPoly.boundingRect() ) )
  {
    ...
    case bsCalligraphic:
    {
      ...
      path.cubicTo( ( centerVec + widthVec - lengthVec * 0.8f ).toPointF(), ( centerVec + 0.4f * widthVec + 0.8f * lengthVec ).toPointF(),
                    centerVec.toPointF() );
      path.cubicTo( ( centerVec - 0.4f * widthVec + 0.8f * lengthVec ).toPointF(), ( centerVec - widthVec - lengthVec * 0.8f ).toPointF(),
                    ( centerVec - widthVec + lengthVec ).toPointF() );

      path.cubicTo( ( centerVec - widthVec - lengthVec * 0.5f ).toPointF(), ( centerVec - 0.2f * widthVec + 1.2f * lengthVec ).toPointF(),
                    ( centerVec + lengthVec * 0.2f ).toPointF() );
      path.cubicTo( ( centerVec + 0.2f * widthVec + 1.2f * lengthVec ).toPointF(), ( centerVec + widthVec - lengthVec * 0.5f ).toPointF(),
                    ( centerVec + widthVec + lengthVec ).toPointF() );
